...
@inline function intensity_point_and_mask(m::N0_wrapper, p)
	...
	pix = Krang.SlowLightIntensityPixel(met, -X, Y, θo * π / 180.0)
...
function Comrade.intensitymap_numeric!(img::Comrade.IntensityMap, m::M) where {M <: VLBISkyModels.ModifiedModel}
	mbase = m.model
...
function Comrade.ComradeBase.intensitymap_numeric!(img::Comrade.IntensityMap, m::KerrGMRF)
	...
	Lx, Ly = size(img)
	for j in 1:3:Ly
		Threads.@threads for i in 1:3:Lx
			tempimg, tempmax = intensity_point_and_mask(m_n0, g[i, j])
			bimg[i, j], mask[i, j] = tempimg * dx * dy, tempmax>0
		end
	end
	#for j in 1:Ly
	#    Threads.@threads for i in 1:Lx
	#        if i%2 == 0 && j%2 == 0
	#            continue
	#        end

	#        bimg[i,j] = intensity_point_and_mask(m_n0, g[i,j])[1]* dx * dy
	#    end
	#end
	...
	m_n1 = N1_wrapper(m)
	for j in 1:3:(Ly-1)
		Threads.@threads for i in 1:3:(Lx-1)

			bimg[i, j+1] = (2bimg[i, j]/3+bimg[i, j+3]/3)
			bimg[i, j+2] = (bimg[i, j]/3+2bimg[i, j+3]/3)
			bimg[i+1, j] = (2bimg[i, j]/3+bimg[i+3, j]/3)
			bimg[i+2, j] = (bimg[i, j]/3+2bimg[i+3, j]/3)
			bimg[i+1, j+1] = (2bimg[i+1, j]/3 + (2bimg[i, j+3]/3+bimg[i+3, j+3]/3)/3)
			bimg[i+1, j+2] = (bimg[i+1, j]/3 + 2(2bimg[i, j+3]/3+bimg[i+3, j+3]/3)/3)
			bimg[i+2, j+1] = (2bimg[i+2, j]/3 + (bimg[i, j+3]/3+2bimg[i+3, j+3]/3)/3)
			bimg[i+2, j+2] = (bimg[i+2, j]/3 + 2(bimg[i, j+3]/3+2bimg[i+3, j+3]/3)/3)


			if mask[i, j] || mask[i+3, j] || mask[i, j+3] || mask[i+1, j+3]
				bimg[i, j] += intensity_point_and_mask(m_n1, g[i, j])[1] * dx * dy
				bimg[i, j+1] += intensity_point_and_mask(m_n1, g[i, j+1])[1] * dx * dy 
				bimg[i, j+2] += intensity_point_and_mask(m_n1, g[i, j+2])[1] * dx * dy 
				bimg[i+1, j] += intensity_point_and_mask(m_n1, g[i+1, j])[1] * dx * dy 
				bimg[i+2, j] += intensity_point_and_mask(m_n1, g[i+2, j])[1] * dx * dy 
				bimg[i+1, j+1] += intensity_point_and_mask(m_n1, g[i+1, j+1])[1] * dx * dy 
				bimg[i+1, j+2] += intensity_point_and_mask(m_n1, g[i+1, j+2])[1] * dx * dy 
				bimg[i+2, j+1] += intensity_point_and_mask(m_n1, g[i+2, j+1])[1] * dx * dy 
				bimg[i+2, j+2] += intensity_point_and_mask(m_n1, g[i+2, j+2])[1] * dx * dy 
			end


		end
		bimg[end, j+1] = 2bimg[end, j]/3+bimg[end, j+3]/3
		bimg[end, j+2] = bimg[end, j]/3+2bimg[end, j+3]/3
	end
	for i in 1:3:(Lx-1)
		bimg[i+1, end] = 2bimg[i, end]/3+bimg[i+3, end]/3
		bimg[i+2, end] = bimg[i, end]/3+2bimg[i+3, end]/3
	end
	return img
end
...
@inline function Krang.emission_coordinates_fast_light(
	...
	return rs, ϕs, νr, νθ, issuccess, (isnan(nmax) ? 0.0 : nmax)
end
