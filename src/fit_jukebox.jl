...
#
# Sample the model
chain = sample(post, NUTS(0.8), 10_000; n_adapts=500, progress=true, initial_params=xopt, adtype=Val(:Enzyme), saveto=DiskStore(), restart=true);

#chain = sample(post, NUTS(0.8), 10_000; n_adapts=500, progress=true, adtype=Val(:Enzyme), saveto=DiskStore(), restart=true);
#
...
CM.image!(axs[2,2], true_intmap, colormap=:afmhot);
