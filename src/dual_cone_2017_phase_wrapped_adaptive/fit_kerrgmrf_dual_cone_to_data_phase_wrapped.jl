...
include(joinpath(dirname(@__DIR__),"models","KerrGMRF_adaptive.jl"))
...
xopt = ((rng, post) -> begin 
    ...
    #truthe= (m_d = μas2rad(3.5))#,frac = 0.1)
...
Comrade.imanalytic(typeof(m))
newgrid = imagepixels(fovx, fovy, npix, npix; executor=ThreadsEx())
intmap = ComradeBase.intensitymap(m, newgrid)
#intmap[begin:2:end, begin:2:end] .= 0

img = imageviz(intmap)#,colorscale=log, colorrange=(1e-10, 1e-2))
display(img)
#end
...
@profview Comrade.logdensityof(fpost, Comrade.inverse(fpost,xopt))

@time Comrade.LogDensityProblems.logdensity_and_gradient(fpost, Comrade.inverse(fpost,xopt))
...
fig = CM.Figure();
#CM.image!(CM.Axis(fig[1,1], xreversed=true, aspect=1), intensitymap(smoothed(msamples[end], μas2rad(10/(2.355))), newgrid), colormapdi=:afmhot)
