...
bl = baselineplot(dvis, :U, :V)
sc = CM.scatter(append!(dvis.config.datatable.U, .-dvis.config.datatable.U) ./ 10^9, append!(dvis.config.datatable.V, .-dvis.config.datatable.V) ./ 10^9);
sc.axis.aspect = 1;
sc.axis.xlabel = "U (Gλ)";
sc.axis.ylabel = "V (Gλ)";
...
angles = begin
    temp = deepcopy(angle.(dvis.measurement))
    append!(temp, temp .+ π )
   temp 
end
rads = begin
    temp = deepcopy.(abs.(dvis.measurement))
    append!(temp, temp)
end
conf = dvis.config
u2017 = conf.datatable.U ./ 10^9
append!(u2017, -u2017)
v2017 = conf.datatable.V ./ 10^9
append!(v2017, -v2017)

colors = begin
    temp = deepcopy(dvis.measurement)
    absvals = abs.(temp)
    #absvals .-= minimum(absvals)
    absvals ./= maximum(absvals) 
    #absvals .*= 255
    temp = CM.Colors.HSL.(angle.(temp) .* 360/(2π), 0.8, absvals .^ (0.7) )
    append!(temp, temp)
    temp
end
fig = CM.Figure(resolution=(600, 600))
ax = CM.Axis(fig[1, 1], xlabel=CM.L"\text{u (G$\lambda$)}", ylabel=CM.L"\text{v (G$\lambda$)}", title=CM.L"\text{M87 April 21, 2018}", aspect=1, titlesize=36, xlabelsize=36, ylabelsize=36)
s = CM.scatter!(ax, u2017, v2017, color = colors)
fig
...
function ModifiedKerrGMRF(θ2, meta) 
    ...
    return m
    #return smoothed(m, μas2rad(5)/(2√(2*log(2))))
end
...
#xopt=transform(fpost, Comrade.inverse(fpost, chain[end-1]))
#
#begin
#	for dir in 1:13
#		logdensity = []
...
#	sample = Comrade.inverse(fpost, xopt)
#	sample[1] += i
#	samplex = Comrade.transform(fpost, sample)
#
#	m = skymodel(post, samplex)
#	newgrid = imagepixels(fovx, fovy, npix, npix; executor = ThreadsEx())
#	ComradeBase.intensitymap(m, newgrid)
#	img = imageviz(ComradeBase.intensitymap(m, newgrid))#,colorscale=log, colorrange=(1e-10, 1e-2))
#	display(img)
